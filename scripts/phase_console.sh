...
cluster_menu() {
  while true; do
    ...
    case "$ACTION" in
      ...
      sampling-analysis)
        ensure_env || return 0
        "${ROOT_DIR}/scripts/potts_analyze.sh" \
          --root "$OFFLINE_ROOT" \
          --project-id "$OFFLINE_PROJECT_ID" \
          --system-id "$OFFLINE_SYSTEM_ID" \
          --cluster-id "$OFFLINE_CLUSTER_ID"
      ...
      delta-energy)
        ...
        pause
        ;;
      hamiltonian-spectral)
        ensure_env || return 0
        "${ROOT_DIR}/scripts/potts_hamiltonian_spectral.sh" \
          --root "$OFFLINE_ROOT" \
          --project-id "$OFFLINE_PROJECT_ID" \
          --system-id "$OFFLINE_SYSTEM_ID" \
          --cluster-id "$OFFLINE_CLUSTER_ID"
      ...
      spectral-intersection)
        ensure_env || return 0
        "${ROOT_DIR}/scripts/potts_spectral_intersection.sh" \
          --root "$OFFLINE_ROOT" \
          --project-id "$OFFLINE_PROJECT_ID" \
          --system-id "$OFFLINE_SYSTEM_ID" \
          --cluster-id "$OFFLINE_CLUSTER_ID"
      ...
      piston-ligand)
        ensure_env || return 0
        "${ROOT_DIR}/scripts/potts_piston_ligand_projection.sh" \
          --root "$OFFLINE_ROOT" \
          --project-id "$OFFLINE_PROJECT_ID" \
          --system-id "$OFFLINE_SYSTEM_ID" \
          --cluster-id "$OFFLINE_CLUSTER_ID"
      ...
      transient-states)
        ensure_env || return 0
        "${ROOT_DIR}/scripts/potts_transient_states.sh" \
          --root "$OFFLINE_ROOT" \
          --project-id "$OFFLINE_PROJECT_ID" \
          --system-id "$OFFLINE_SYSTEM_ID" \
          --cluster-id "$OFFLINE_CLUSTER_ID"
      ...
      lambda-sweep)
        ensure_env || return 0
        "${ROOT_DIR}/scripts/potts_lambda_sweep.sh" \
          --root "$OFFLINE_ROOT" \
          --project-id "$OFFLINE_PROJECT_ID" \
          --system-id "$OFFLINE_SYSTEM_ID" \
          --cluster-id "$OFFLINE_CLUSTER_ID"
      ...
      potts-nn)
        ensure_env || return 0
        "${ROOT_DIR}/scripts/potts_nn_mapping.sh" \
          --root "$OFFLINE_ROOT" \
          --project-id "$OFFLINE_PROJECT_ID" \
          --system-id "$OFFLINE_SYSTEM_ID" \
          --cluster-id "$OFFLINE_CLUSTER_ID"
      ...
      backmapping-dataset)
        ensure_env || return 0
        "${ROOT_DIR}/scripts/backmapping_dataset.sh" \
          --root "$OFFLINE_ROOT" \
          --project-id "$OFFLINE_PROJECT_ID" \
          --system-id "$OFFLINE_SYSTEM_ID" \
          --cluster-id "$OFFLINE_CLUSTER_ID"
