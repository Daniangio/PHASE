...
if [ "$FIT_METHOD" != "pmi" ]; then
  ...
  PLM_LR="$(prompt "PLM lr" "1e-3")"
  PLM_LR_MIN="$(prompt "PLM lr min" "1e-5")"
  ...
  PLM_GRAD_ACCUM_STEPS="$(prompt "PLM grad accumulation steps" "1")"
...
if [ "$FIT_METHOD" != "pmi" ]; then
  CMD+=(
    ...
    --plm-grad-accum-steps "$PLM_GRAD_ACCUM_STEPS"
